...
struct SceneEntry {
    bool used = false;
    uint8_t area = 0;   // 1 - relay channel
    uint8_t scene = 0;  // 0 - (relay channel * 2) (0 = "stop scene", handled by caller, not stored)
    bool relayState[RELAY_CHANNEL_COUNT] = {false, false, false, false};
};
...
public:
    ...
    virtual bool lookup(uint8_t area, uint8_t scene, bool outStates[RELAY_CHANNEL_COUNT]) const = 0;

    // Adds or overwrites the mapping for area/scene. Returns false if the
    // table is full and area/scene wasn't already present.
    virtual bool set(uint8_t area, uint8_t scene, const bool states[RELAY_CHANNEL_COUNT]) = 0;
...
public:
    bool lookup(uint8_t area, uint8_t scene, bool outStates[RELAY_CHANNEL_COUNT]) const override {
        for (const auto& e : entries_) {
            if (e.used && e.area == area && e.scene == scene) {
                for (uint8_t i = 0; i < RELAY_CHANNEL_COUNT; ++i) outStates[i] = e.relayState[i];
                return true;
            }
        }
        return false;
    ...
    bool set(uint8_t area, uint8_t scene, const bool states[RELAY_CHANNEL_COUNT]) override {
        for (auto& e : entries_) {
            if (e.used && e.area == area && e.scene == scene) {
                for (uint8_t i = 0; i < RELAY_CHANNEL_COUNT; ++i) e.relayState[i] = states[i];
                return true;
            }
        }
        for (auto& e : entries_) {
